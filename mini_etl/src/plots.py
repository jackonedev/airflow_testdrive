...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    sns.scatterplot(data=df, x="product_weight_g", y="freight_value").set(
        xlabel="product_weight_g", ylabel="freight_value"
    )
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    ax1.plot(df["date"], df["order_count"], color="green")
    [
        ax1.axvline(x=holy, color="blue", linestyle="dotted", alpha=0.5)
        for holy in df.loc[df.holiday == True, "date"].to_list()
    ]
    plt.show()
