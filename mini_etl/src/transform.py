...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    orders["order_purchase_timestamp"] = pd.to_datetime(
        pd.to_datetime(orders["order_purchase_timestamp"]).dt.to_period("D").astype(str)
    )

    holidays["date"] = pd.to_datetime(
        pd.to_datetime(holidays["date"]).dt.to_period("D").astype(str)
    )

    # Filtering only the order purchase timestamps from the year 2017.
    filtered_dates = orders[orders["order_purchase_timestamp"].dt.year == 2017].rename(
        columns={"order_purchase_timestamp": "date"}
    )

    # Counting the orders per day.
    order_purchase_ammount_per_date = (
        filtered_dates.groupby("date")
        .agg(order_count=("order_id", "count"))
        .reset_index()
    )

    # Creating a dataframe with the result. Assign it to `result_df` variable.
    result_df = order_purchase_ammount_per_date[["order_count", "date"]].copy()
